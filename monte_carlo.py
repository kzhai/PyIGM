...
class MonteCarlo(object):
	...
	def _initialize(self, data, alpha_alpha=1., alpha_kappa=1., alpha_nu=1., alpha_mu=None, alpha_lambda=None):
		self._counter = 0;
		...
		lambda_n = self._alpha_lambda + S + self._alpha_kappa * self._count[0] * numpy.dot(mu_x_mu_0.transpose(), mu_x_mu_0) / (
			self._alpha_kappa + self._count[0]);
	...
	def learning(self):
		...
		for (point_counter, point_index) in enumerate(order):
			...
			if self._count[old_label] == 0:
				...
				lambda_n = self._alpha_lambda + S + self._alpha_kappa * self._count[old_label] * numpy.dot(mu_y_mu_0.transpose(), mu_y_mu_0) / (self._alpha_kappa + self._count[old_label]);
			...
			for k in xrange(self._K):
				...
				log_prob -= (
					scipy.special.gammaln(nu / 2.) + (self._D / 2.) * numpy.log(nu) + (self._D / 2.) * numpy.log(
						numpy.pi));
			...
			if old_label == new_label:
				...
				mu_y_mu_0 = mu_y - self._mu_0;
				assert (numpy.dot(mu_y_mu_0.transpose(), mu_y_mu_0).shape == (self._D, self._D));
				lambda_n = self._alpha_lambda + S + self._alpha_kappa * self._count[new_label] * numpy.dot(
					mu_y_mu_0.transpose(), mu_y_mu_0) / (self._alpha_kappa + self._count[new_label]);
				sigma = (lambda_n * (kappa_n + 1)) / (kappa_n * nu);
				assert (numpy.linalg.det(sigma) > 0);
	...
	def export_snapshot(self, directory):
		...
		numpy.savetxt(os.path.join(directory, "Mu-" + str(self._counter)),
		              self._sum[:self._K, :] / self._count[:self._K][numpy.newaxis, :].transpose());
		sigma = self._sigma_inv;
		for k in xrange(self._K):
			sigma[k, :, :] = numpy.linalg.inv(sigma[k, :, :]);
		numpy.savetxt(os.path.join(directory, "Sigma-" + str(self._counter)),
		              numpy.reshape(sigma[:self._K, :, :], (self._K, self._D * self._D)));
		vector = numpy.array([self._alpha_alpha, self._alpha_kappa, self._alpha_nu]);
		numpy.savetxt(os.path.join(directory, "Hyperparameter-vector-" + str(self._counter)), vector);
		matrix = numpy.vstack((self._mu_0, self._alpha_lambda));
